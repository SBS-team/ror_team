...
set :application, "ror_team-dev"
set :rails_env, "production"
set :domain, "admintools@192.168.137.1" # Это необходимо для деплоя через ssh. Именно ради этого я настоятельно советовал сразу же залить на сервер свой ключ, чтобы не вводить паролей.
...
set :repository,  "git@github.com:is-valid/ror_team.git" # Путь до вашего репозитария. Кстати, забор кода с него происходит уже не от вас, а от сервера, поэтому стоит создать пару rsa ключей на сервере и добавит$
set :branch, "dev" # Ветка из которой будем тянуть код для деплоя.
