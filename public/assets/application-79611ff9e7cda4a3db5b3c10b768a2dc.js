    ...
    return $("#new_resume").validate({
      rules: {
        ...
        "resume[description]": {
          required: {
            depends: function() {
              return $.isEmptyObject($("[name='resume[upload_files_attributes][filename]']").val());
            }
          },
          maxlength: 40,
          minlength: 2
  ...
  $(document).ready(function() {
    return $("#new_message").validate({
      ...
      errorPlacement: function(error, element) {
        if (element.attr("name") === "message[service_type]") {
          error.insertBefore($("div[class=icons]"));
        }
        if (element.attr("name") === "message[work_type]") {
          error.insertBefore($("div[class=choose]"));
        }
        if (element.attr("name") === "message[email]") {
          error.insertAfter($("input[id=message_email]"));
        }
        if (element.attr("name") === "message[phone]") {
          error.insertAfter($("input[id=message_phone]"));
        }
        if (element.attr("name") === "message[name]") {
          return error.insertAfter($("input[id=message_name]"));
        }
      }
    });
  });
...
//jQuery.validator.addMethod("phone_validation_allow_empty_number", function(value){
//    value = value.replace(/[\(\)\-]/g, "");
//    if ((/\+?\d{10,}$/.test(value))||(/^\s*$/.test(value))) return true
//    else return false;
